  ...
  return (
    <div className='flex-none w-80 items-center justify-center max-w-lg'>
      <section className='p-5 m-3 border rounded-md'>
        <Typography variant='h4' className='mb-2'>いいね数ランキング</Typography>
        {userRanks?.map((userRank,index) => (
            <div key={userRank.favoriteUser} className='flex justify-between shadow-blue-700'>
                <span className='flex'>
                ...
                <Typography>{userRank.count}</Typography>
            </div>
        ))}
      </section>
    </div>
