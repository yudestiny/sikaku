...
const UserRank = ({userRanks}) => {
  return (
    <div className='flex w-80 items-center max-w-lg'>
      <section className='p-5 m-3 border rounded-md space-y-2'>
        <Typography variant='h4' className='mb-3'>いいね数ランキング</Typography>
        {userRanks?.map((userRank,index) => (
            <div key={userRank.favoriteUserId} className='flex gap-3 min-w-full shadow-blue-700'>
                ...
                <Link to={`/my-page/${userRank.favoriteUserId}`}>
                  <div className='hover:cursor-grab flex w-full justify-between items-center hover:underline gap-2'>
                    <div className='gap-2 justify-between flex'>
                      <ProfileImg img={userRank.image} size={6} />
                      <Typography>{userRank.name}</Typography>
                    </div>
                    <Typography className='justify-end flex'>{userRank.count}</Typography>
                  </div>
                </Link>
            </div>
        ))}
